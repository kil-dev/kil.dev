...
function TooltipContent({
  ...
  const content = (
    <TooltipPrimitive.Content
      ...
      className={cn(
        'relative z-[200] w-fit origin-(--radix-tooltip-content-transform-origin) animate-in overflow-visible rounded-md bg-primary px-3 py-1.5 text-xs text-balance text-primary-foreground fade-in-0 zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95',
        className,
      )}
      {...props}>
      <div className="kd-tooltip-inner">{children}</div>
      <TooltipPrimitive.Arrow className="pointer-events-none z-0 size-2.5 translate-y-[calc(-50%_-_2px)] rotate-45 rounded-[2px] bg-primary fill-primary" />
    </TooltipPrimitive.Content>
  )
